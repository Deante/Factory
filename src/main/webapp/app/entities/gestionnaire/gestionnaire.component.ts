...
export class GestionnaireComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.gestionnaireService.search({
    ...
    trackId(index: number, item: Gestionnaire) {
        return item.id;
    }
    registerChangeInGestionnaires() {
        this.eventSubscriber = this.eventManager.subscribe('gestionnaireListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.gestionnaires.push(data[i]);
        }
    }
