...
export class DisponibiliteComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.disponibiliteService.search({
        ...
        this.disponibiliteService.query({
            page: this.page,
            size: this.itemsPerPage,
            sort: this.sort()
        }).subscribe(
    ...
    registerChangeInDisponibilites() {
        this.eventSubscriber = this.eventManager.subscribe('disponibiliteListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.disponibilites.push(data[i]);
        }
    }
